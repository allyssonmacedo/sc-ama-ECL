...
$.File_Persons.Layout Bulkup($.File_Persons_Slim.Layout L,
  ...
  SELF.MiddleName := Std.Str.ToTitleCase(L.NameSuffix);
  SELF := R;
  SELF := L;
END;
BulkRecs := JOIN($.File_Persons_Slim.File, $.File_LookupCSZ.File,
  LEFT.CSZ_ID =RIGHT.CSZ_ID, 
  Bulkup(LEFT,RIGHT), LEFT OUTER, LOOKUP);
  
OUTPUT(BulkRecs,, '~ONLINE::allysson::OUT::Persons_Rejoined', OVERWRITE);
